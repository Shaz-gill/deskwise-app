...
`packages/server` has Prisma and Better Auth wired up end-to-end, plus a first real admin feature — server-side user management: the Better Auth handler is mounted at `/api/auth` (behind a credential-path rate limiter), `cors` (origin-checked against a shared trusted-origins allowlist) and `helmet` are both applied, and `requireAuth`/`requireAdmin` middleware gate an `/api/users` CRUD router (list/create/edit/soft-delete). There's also a new `packages/core` workspace package — currently just Zod schemas (`createUserSchema`, `editUserSchema`, `loginSchema`) — that `packages/server/routes/users.ts` imports from (`../../core`); `packages/client` doesn't consume it yet and still has its own separate copy of `loginSchema` in `LoginPage.tsx`. `packages/client` has a working auth flow on top of the Vite template: shadcn/ui (Tailwind v4, `base-nova` style, `@base-ui/react` primitives) plus `react-router-dom` route guards (`ProtectedRoute`, `GuestRoute`, `AdminRoute`, under `src/components/routes/`) gating a `/login` page (react-hook-form + zod), a `/` home page, and a placeholder admin-only `/users` page — all wrapped in a shared `Layout`/`NavBar`. `UsersPage` is still just a heading client-side — the `/api/users` endpoints exist server-side but have no UI consuming them yet. Beyond auth and user management, none of the other product features in `docs/project-scope.md` (tickets, AI classification, email ingestion) are implemented yet. `docs/implementation-plan.md` has the intended build order (auth → user management → ticket CRUD → AI features → email → dashboard), and `docs/tech-stack.md` has the intended stack (Postgres + Prisma, session auth, SendGrid/Mailgun, Claude API). Check these docs for intent before assuming current code reflects the target design — e.g. `packages/server/.env.example` used to reference `OPENAI_API_KEY` even though the tech stack doc specifies Claude/Anthropic (now replaced by `DATABASE_URL`/`BETTER_AUTH_SECRET`).
...
- Auth: Better Auth (`packages/server/lib/auth.ts`), configured with the Prisma adapter and email/password sign-in with `disableSignUp: true` (users are pre-provisioned, not self-registered — matches the admin-creates-agents model in `docs/project-scope.md`). `trustedOrigins` is wired from the `TRUSTED_ORIGINS` env var (`.env` has `http://localhost:*` for dev) — without it, any authenticated POST from the browser (sign-out, etc.) gets rejected with a 403 `Invalid Origin`, since Better Auth's origin check is separate from Express's `cors()`. `user.additionalFields.role` exposes the `role` column (see below) on the session/`get-session` user object — Better Auth silently drops fields it doesn't know about, so this must stay declared here (and mirrored client-side, see below) for `role` to ever reach the client. Its `User`/`Session`/`Account`/`Verification` models live in the same `schema.prisma` as the rest of the app's data; `User.role` is a `Role` enum (`admin` | `agent`, default `agent`) added for the admin-vs-agent model. Mounted in `index.ts` at `/api/auth` via `toNodeHandler(auth)`, registered *before* `express.json()`. Route protection is done with `packages/server/middleware/require-auth.ts` (`requireAuth`), which calls `auth.api.getSession` and attaches `req.user`/`req.session` — typed via the global augmentation in `packages/server/express.d.ts`. `packages/server/middleware/require-admin.ts` (`requireAdmin`) further gates on `req.user.role === 'admin'`, always chained after `requireAuth` (it reads `req.user` without a null check, so it assumes `requireAuth` already ran). `packages/server/middleware/auth-limiter.ts` defines an `authLimiter` (`express-rate-limit`, 20 req/15min) scoped to credential-related paths (`/sign-in`, `/sign-up`, `/change-password`, `/change-email`) and is mounted ahead of the Better Auth handler in `index.ts`. `packages/server/lib/trusted-origins.ts` parses `TRUSTED_ORIGINS` (supporting `*` wildcards, e.g. `http://localhost:*`) into a single `trustedOrigins` list/`isTrustedOrigin()` check shared by both Better Auth's `trustedOrigins` config and the global `cors()` origin callback in `index.ts`, replacing what used to be two separate parses of the same env var. `User` soft-delete: `User.deletedAt` (nullable `DateTime`) was added to `schema.prisma` and is checked in `auth.ts`'s `hooks.before` (blocks sign-in for a soft-deleted user with a `FORBIDDEN` `APIError`) and in the `/api/users` delete route (sets `deletedAt` + revokes sessions instead of a hard delete) — **as of this writing the schema change hasn't been migrated (`bunx prisma migrate dev`) or regenerated (`bunx prisma generate`)**, so `bunx tsc --noEmit` currently fails on `deletedAt` not existing on the generated `User` types; run both before relying on this code. The old placeholder `GET /` and `GET /api/hello` routes and the `GET /api/me` route are gone from `index.ts`, replaced by the `/api/users` router.
- `packages/core`: a new workspace package (`packages/core/src/schemas.ts`, re-exported from `src/index.ts`), holding shared Zod schemas — `loginSchema`, `createUserSchema`, `editUserSchema`. Only `packages/server` imports from it so far (via the relative path `../../core`, not a `node_modules`/workspace-name import); `packages/client`'s `LoginPage.tsx` still has its own local `loginSchema` copy rather than importing the shared one.
