...
- `packages/client`: Vite + React 19 + TypeScript, shadcn/ui + Tailwind v4 (see `docs/shadcn-ui-setup.md` above). `src/lib/auth-client.ts` creates the Better Auth React client (`authClient`, from `better-auth/react`) with an `inferAdditionalFields` plugin declaring `role` — this must stay in sync with `user.additionalFields` in `packages/server/lib/auth.ts` or `session.user.role` silently comes back `undefined`. Routing is `react-router-dom`, wired in `main.tsx` (`BrowserRouter`) and `App.tsx` (route table): `ProtectedRoute` gates `/` on a session, `GuestRoute` gates `/login` on the *absence* of one, and `AdminRoute` further gates `/users` on `role === 'admin'` — all three read `authClient.useSession()` directly rather than passing user/session down as props. `Layout` + `NavBar` provide the shared authenticated app shell.
